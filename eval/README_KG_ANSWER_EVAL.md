...
- 样本文本：`eval/kg_gold_demo/kg_gold_demo_page_0044.txt`，内容为第 44 页完整 OCR 文本。
...
```bash
./.venv/bin/python scripts/build_kg_with_deepseek.py --input eval/kg_gold_demo/kg_gold_demo_page_0044.txt --dry-run --output output/kg_gold_demo_page44_extraction.json
```

第二步，运行 KG Gold Evaluation：

```bash
./.venv/bin/python scripts/evaluate_kg_gold.py --extraction output/kg_gold_demo_page44_extraction.json --gold eval/kg_gold.example.jsonl --output output/kg_gold_demo_page44_eval.json
```

如果要评价大批量抽取结果，把 `--extraction` 换成对应 pipeline 输出：

```bash
./.venv/bin/python scripts/evaluate_kg_gold.py --extraction output/pipeline_20260522_122022_kg_extraction.json --gold eval/kg_gold.example.jsonl --output output/kg_gold_eval.json
```
...
| `--dry-run` | 只输出抽取 JSON，不写入 Neo4j。适合离线评估。 | 开关，无参数值 |
...
- `output/graphrag_mode_compare.md`： Markdown 对比表。
- `output/graphrag_mode_compare.csv`：对比表。
...
```bash
./.venv/bin/python scripts/evaluate_graphrag_retrieval.py --dataset eval/retrieval_eval.example.jsonl --ks 3,5,10 --graph-ks 5,10 --mode vector --output output/retrieval_eval_vector.json
./.venv/bin/python scripts/evaluate_graphrag_retrieval.py --dataset eval/retrieval_eval.example.jsonl --ks 3,5,10 --graph-ks 5,10 --mode graph --output output/retrieval_eval_graph.json
./.venv/bin/python scripts/evaluate_graphrag_retrieval.py --dataset eval/retrieval_eval.example.jsonl --ks 3,5,10 --graph-ks 5,10 --mode hybrid --judge-answer --output output/retrieval_eval_hybrid_judge.json
```
...
| `--python` | 指定 Python 解释器，默认使用当前工程 `.venv`。 | `./.venv/bin/python` |
...
```bash
./.venv/bin/python scripts/ingest_source_pipeline.py --file "催化裂化装置操作指南.pdf" --pages 30-80 --workers 3 --force-ocr --page-window 20 --page-window-overlap 2
```

如果需要临时恢复旧的一页一个 txt 行为：

```bash
./.venv/bin/python scripts/ingest_source_pipeline.py --file "催化裂化装置操作指南.pdf" --pages 30-80 --workers 3 --force-ocr --page-window 0
```
